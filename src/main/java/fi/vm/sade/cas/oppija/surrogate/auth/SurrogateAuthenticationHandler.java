...
public class SurrogateAuthenticationHandler implements AuthenticationHandler {
    ...
    private Principal createPrincipal(SurrogateAuthenticationDto dto) {
        ...
        dto.impersonatorData.principalAttributes.entrySet().stream()
                .forEach(entry -> attributes.put(impersonatorAttributeKey(entry.getKey()), Arrays.asList(entry.getValue())));
        attributes.put(ATTRIBUTE_NAME_NATIONAL_IDENTIFICATION_NUMBER, Arrays.asList(dto.nationalIdentificationNumber));
        try {
            personService.findOidByNationalIdentificationNumber(dto.nationalIdentificationNumber)
                    .ifPresent(oid -> attributes.put(ATTRIBUTE_NAME_PERSON_OID, Arrays.asList(oid)));
        } catch (Exception e) {
            LOGGER.error("Unable to get oid by national identification number", e);
        }
        attributes.put(ATTRIBUTE_NAME_PERSON_NAME, Arrays.asList(dto.name));
