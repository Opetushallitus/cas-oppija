...
public class RegisteredServiceConfiguration {
    ...
    private static class CasOppijaAttributeReleasePolicy extends ReturnAllAttributeReleasePolicy {
        ...
        protected Map<String, List<Object>> returnFinalAttributesCollection(
                Map<String, List<Object>> attributesToRelease, RegisteredService service) {
            // pac4j adds both saml name (e.g. urn:oid:2.5.4.3) and friendly name (e.g. cn)
            // to principal attributes
            // and cas converts all attributes containing ":" into numbers which is not
            // allowed in xml tags
    ...
    public List<RegisteredService> inMemoryRegisteredServices(Environment environment) {
        ...
        // DefaultRegisteredServiceAccessStrategy strategy = new
        // DefaultRegisteredServiceAccessStrategy();
        // DefaultRegisteredServiceDelegatedAuthenticationPolicy policy = new
        // DefaultRegisteredServiceDelegatedAuthenticationPolicy(new
        // LinkedHashSet<>(Arrays.asList("Suomi.fi", "Fakesuomi.fi")), true, false);
